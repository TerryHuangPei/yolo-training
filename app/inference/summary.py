...
class InferenceSummary:
    ...
    def payload(self) -> dict[str, Any]:
        return {
            ...
            "detections_by_class": dict(self.counts),
            "average_confidence_by_class": {
                key: sum(values) / len(values) for key, values in self.confidences.items()
            },
            "inference_ms_average": sum(self.times) / len(self.times) if self.times else 0,
