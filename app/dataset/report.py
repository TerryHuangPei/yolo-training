...
class DatasetReport:
    ...
    def write(self, reports_dir: Path) -> None:
        ...
        (reports_dir / "dataset-report.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
        lines = [f"Images: {self.image_count}", f"Labels: {self.label_count}", f"Fatal errors: {self.fatal_count}"]
        lines.extend(f"[{item.severity}] {item.path}{':' + str(item.line) if item.line else ''} {item.message}" for item in self.issues)
