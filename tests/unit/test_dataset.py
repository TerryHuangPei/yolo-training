...
@pytest.mark.parametrize(
    "label", ["0 0.5 0.5 0.2", "1 0.5 0.5 0.2 0.2", "0 nan 0.5 0.2 0.2", "0 0.95 0.5 0.2 0.2"]
)
...
def test_duplicate_and_leakage(tmp_path: Path) -> None:
    ...
    (root / "labels" / "val").mkdir()
    (root / "images" / "val" / "a.jpg").write_bytes(
...
def test_zip_slip(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(archive, "w") as handle:
        handle.writestr("../bad", "x")
    with pytest.raises(DatasetExtractionError):
        extract_dataset(archive, tmp_path / "out", Settings())
...
def test_roboflow_layout_is_normalized(tmp_path: Path) -> None:
    root = tmp_path
    for split in ("train", "valid"):
        ...
        label_dir.mkdir(parents=True)
        Image.new("RGB", (10, 10)).save(image_dir / "a.jpg")
        (label_dir / "a.txt").write_text("0 0.5 0.5 0.2 0.2", encoding="utf-8")
    (root / "data.yaml").write_text("names: [helmet]\n", encoding="utf-8")
    layout = detect_dataset(root)
    assert layout.split
    assert layout.classes == ["helmet"]
    assert (root / "images" / "val" / "a.jpg").exists()
